                    ...
                    <div class="x_content">
                        ...
                        <form class="form-horizontal form-label-left" action="{{ route('masters.store') }}"
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-6 col-sm-6">
                                    <label class="control-label fs18">Parent <span class="text-danger">*</span></label>
                                ...
                                <div class="col-md-12 col-sm-12">
                                    ...
                                    <textarea name="desc" id="desc" class="w-100"
                                        rows="6">{{ $master->desc }}</textarea>
                                </div>
                                <div class="col-md-6 col-sm-6 ">
                                    <label class="control-label fs18">Status <span class="text-danger">*</span></label>
                                    <select class="form-control" name="status">
                                        <option {{ $master->status == 1 ? 'selected' : '' }} value="1">Active
                                        </option>
                                        <option {{ $master->status == 0 ? 'selected' : '' }} value="0">Deactive
                                        </option>
                                    </select>
                                ...
                                <div class="col-md-6 col-sm-6">
                                    ...
                                    <input type="text" class="form-control" placeholder="Order" id="order" name="order">
                                </div>
                            </div>
...
<script>
    $("#name").stringToSlug();
        $(document).ready(function() {
            $.ajax({
                url: "/get-types?typeId={{ $master->type_id }}",
                type: "GET",
                dataType: 'json',
                success: function(result) {
                    ...
                    }).val({{ $master->type_id }}).trigger("change");
                    $.ajax({
                        url: "/get-table-type-by-type-id?type={{ $master->type_id }}",
                        type: "GET",
                        dataType: 'json',
                        success: function(result) {
                            $("#table_type").html('');
                            $.each(result, function(key, value) {
                                var table_type = {{ $master->table_type }};
                            ...
                            }).val({{ $master->table_type }}).trigger("change");
                            $.ajax({
                                url: "/get-parent-by-table-type?type={{ $master->type_id }}&table_type={{ $master->table_type }}",
                                type: "GET",
                                dataType: 'json',
                                success: function(result) {
                                    $("#parent").html('');
                                    ...
                                    }).val({{ $master->parent }}).trigger("change");
                                }
                            });
                        }
                    });
